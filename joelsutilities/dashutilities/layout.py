...
EL_MAP = {
    ...
    "element-div": {"dash_cls": html.Div},
    "element-input": {"dash_cls": dbc.Input},
    "element-progress": {"dash_cls": dbc.Progress},
    "element-input-group": {"dash_cls": dbc.InputGroup},
    "element-input-group-addon": {"dash_cls": dbc.InputGroupAddon},
    "element-paragraph": {"dash_cls": html.P},
}
...
def gen_table(spec: Dict) -> dbase.Component:
    ...
    if not spec.pop("no_fixed_widths", None):
        style_cell |= {"maxWidth": 0}  # fix column widths
    return html.Div(
        dash_table.DataTable(
            id=table_id,
            columns=[dict(name=v, id=k) for k, v in table_cols.items()],
...
def gen_modal(spec: Dict) -> dbase.Component:
    ...
    header_children = (
        _gen_element(header_spec) if isinstance(header_spec, list) else header_spec
    )
    ...
    return dbc.Modal(
        [dbc.ModalHeader(header_children), dbc.ModalFooter(footer_children)],
        id=modal_id,
    )
...
def gen_periodic(spec: Dict) -> dbase.Component:
    return dcc.Interval(id=spec.pop("id"), interval=spec.pop("interval_milliseconds"))
...
def generate_sidebar(spec: SidebarSpec):
    ...
    for row_spec in sidebar_content_spec:
        children.append(
            html.Div(
                _gen_element(row_spec), className=f"mb-{SIDE_EACH_MB} mx-{SIDE_EACH_MX}"
            )
        )
    return html.Div(
        html.Div(
...
def generate_container(spec: Dict):
    ...
    if not isinstance(content_spec, list):
        raise LayoutException(
            f'expected content to be list, instead got "{type(content_spec)}"'
        )
    for row_spec in content_spec:
        if isinstance(row_spec, list):
            ...
            cont_children.append(dbc.Row(row_children, align="center"))
        elif isinstance(row_spec, dict):
            cont_children.append(_gen_element(row_spec))
        else:
            raise LayoutException(
                f'expected row spec list/dict, got "{type(row_spec)}"'
            )
    return html.Div(
        html.Div(cont_children, className="d-flex flex-column h-100"),
...
def generate_header(title, left_spec, right_spec):
    return dbc.Row(
        [
            dbc.Col(_gen_element(left_spec) if left_spec else None, width=3),
            dbc.Col(
                dbc.Row(
            ...
            dbc.Col(_gen_element(right_spec) if right_spec else None, width=3),
        ],
        align="center",
        ...
        [
            ...
            html.Div(
                [
                    dcc.Store(
                        s["id"],
                        storage_type=s.get("storage_type", "session"),
