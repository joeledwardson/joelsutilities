...
from joelsutilities.exceptions import DashUtilsException
...
def assign_value(spec: Dict[str, T], args: List[Any]) -> Dict[str, T]:
    ...
    if isinstance(spec, list):
        ...
        if not len(args):
            raise DashUtilsException(
                f'cannot assign value for spec: "{spec}", args empty'
            )
        return args.pop(0)
...
def dict_callback(
    ...
    def outer(process: Callable[[TDict, TDict, TDict], None]):
        ...
        def callback(*args):
            ...
            if len(args):
                raise DashUtilsException(
                    f"still have {len(args)} args remaining to process"
                )
