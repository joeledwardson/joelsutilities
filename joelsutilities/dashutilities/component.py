...
from .core import CSSClassHandler, triggered_id
from .layout import BTN_COLOR, ContentSpec, StoreSpec
...
def wrapper(wrapper_id, content):
    return {"type": "element-div", "id": wrapper_id, "children_spec": content}
...
def nav_element(
    ...
    return {
        ...
        "id": nav_id,
        "children_spec": [
            {"type": "element-fontawesome", "css_classes": icon + " mr-2"},
            {
                "type": "element-div",
...
def components_layout(
    components: List[Component], title: str, config: ConfigParser
) -> ContentSpec:
    def not_none(lst):
    ...
    return ContentSpec(
        **{
                ...
                "children_spec": [
                    {
                        ...
                        "children_spec": [
                            {"type": "element-div", "id": c.LOADING_ID}
                            for c in components
                            if c.LOADING_ID
                        ],
                    },
                    {"type": "element-div", "css_classes": "flex-grow-1"},
            ...
            "hidden_elements": list(
                itertools.chain(*[c.modal_specs(config) for c in components])
            ),
            "containers": not_none([c.display_spec(config) for c in components]),
            "sidebars": not_none([c.sidebar(config) for c in components]),
            "stores": list(
                itertools.chain(*[c.additional_stores() for c in components])
            )
            + [{"id": c.NOTIFICATION_ID} for c in components if c.NOTIFICATION_ID],
            "tooltips": list(
                itertools.chain.from_iterable(c.tooltips(config) for c in components)
            ),
        }
    )
